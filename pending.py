...
def manage_pending_signal(state, instruments, expiry, smart_api, prev_day, prev_day_cache, today_start, run_cache=None):
    ...
    function's docstring in market_data.py for the full root-cause
    writeup. This function only ever runs on a run where state has a
    resting pending_signal, so it's typically the ONLY consumer of the
    sell-strike token that run; run_cache mainly matters here in case
    priming already warmed it, or in the rare same-run overlap where the
    buy engine also happens to touch the same underlying strike.
