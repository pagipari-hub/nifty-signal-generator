...
def _is_fresh_pdl_breakdown_signal(df, pdl):
    """
    NEW (2026-08-13, PDL fallback entry -- sell side only). TRANSITION
    check, same shape/reasoning as is_fresh_crossover_signal() above:
    did the latest CLOSED candle's close just cross below PDL, with the
    PREVIOUS candle's close still at/above it -- not merely "close is
    below PDL right now", which could otherwise re-fire a brand new
    pending_signal every single run for as long as price stays under
    PDL after a prior PDL-sourced signal already cancelled/expired, with
    no actual new breakdown having happened. Same class of bug
    ...
    today, same fail-open-to-"no signal" posture as everything else in
    this codebase when required data is missing.
    """
    if df is None or len(df) < 2 or pdl is None:
        return False
...
def scan_for_new_signal(state, leg_pairs, instruments, expiry, smart_api, prev_day, prev_day_cache, today_start, run_cache=None):
    ...
    NEW (2026-08-13, PDL fallback entry -- sell side only): when
    EMA/VWAP does NOT fire for a leg this run, that same leg is now also
    checked for a fresh PDL breakdown (see _is_fresh_pdl_breakdown_signal()
    above) before moving to the next leg. EMA/VWAP always has priority
    on a same-run collision -- this is checked in an elif, only reached
    when is_fresh_crossover_signal() was False for this leg this run.
    PDL fallback is deliberately NOT run through the squeeze gate above
    -- that gate's whole premise (a tight EMA5/EMA25/VWAP bunching
    producing low-conviction whipsaw crossovers) doesn't apply to a
    confirmed-close breakdown signal, which is a different kind of price
    action entirely. Uses state["daily_pdl"][leg["option_type"]],
    written once per day by candle_priming.lock_daily_pdl() -- None if
    ...
    for leg in leg_pairs:
        ...
        if is_fresh_crossover_signal(df):
            ...
            hedge_token_info = ac.resolve_option_token(instruments, expiry, leg["hedge_strike"], leg["option_type"])
            if not hedge_token_info:
                print(f"PDL breakdown fired on {sell_token_info['symbol']} but hedge strike "
                      f"{leg['hedge_strike']}{leg['option_type']} could not be resolved -- skipping this signal.",
                      file=sys.stderr)
                continue
            ...
            if hedge_entry_price is None:
                print(f"Could not fetch hedge leg LTP for {hedge_token_info['symbol']} at PDL signal time -- "
                      "hedge_entry_price will be logged as null; P&L for this trade will be incomplete.",
                      file=sys.stderr)
            ...
            print(f"PENDING SIGNAL [PDL breakdown]: SELL {p['sell_symbol']} resting limit @ "
                  ...
                  f"PDL={pdl:.2f})")
            print(
                f"[DEBUG] scan_for_new_signal: stopping after {p['sell_symbol']} (PDL fallback) -- "
                "remaining leg(s) in this run's leg_pairs were not scanned "
                "(single pending_signal slot in state.json).",
                file=sys.stderr,
            )
